  ...
  return {
    importMedia: async (file: File) => {
      ...
      try {
        ...
        // Adapt the PROJECT CANVAS to the first imported video/image so the
        // preview frame matches the source aspect ratio (portrait, square,
        // 4:3, etc.) instead of letterboxing it inside the default 16:9 canvas.
        // This only fires for the very first media in an otherwise-empty
        // project — it never overrides a canvas the user has already worked in.
        // NOTE: this sets the project/export resolution to the source's exact
        // intrinsic dimensions; it does not resize/crop/stretch the media.
        const isFirstMedia = project.mediaLibrary.items.length === 0;
        const srcW = newMediaItem.metadata.width;
        const srcH = newMediaItem.metadata.height;
        const hasSourceDimensions =
          ...
          srcW > 0 &&
          srcH > 0;
        const canvasDiffersFromSource =
          project.settings.width !== srcW || project.settings.height !== srcH;
        const shouldAdaptCanvas =
          isFirstMedia && hasSourceDimensions && canvasDiffersFromSource;

        const nextSettings = shouldAdaptCanvas
          ? { ...project.settings, width: srcW, height: srcH }
          : project.settings;
