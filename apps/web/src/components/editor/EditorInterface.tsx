...
// Small chevron button pinned to a panel's inner edge that collapses it.
const PanelCollapseButton: React.FC<{
  ...
  <button
    type="button"
    onClick={onClick}
    aria-label={side === "left" ? "Collapse media panel" : "Collapse properties panel"}
    title="Collapse panel"
    className={`absolute top-2 z-20 flex h-6 w-6 items-center justify-center rounded-md text-fg-muted hover:bg-hover hover:text-fg transition-colors ${
...
// Thin vertical strip shown in place of a collapsed panel, with an expand tab.
const CollapsedStrip: React.FC<{
  ...
  <button
    ...
    title={`Expand ${label}`}
    className="group flex h-full w-full flex-col items-center gap-3 py-3 text-fg-muted hover:bg-hover hover:text-fg transition-colors"
  >
...
// from the mockup is `1fr var(--tl-height)` rows. A smaller default
// timeline gives the player/stage a larger share (Studio-style big preview);
// users can still drag the handle up to enlarge the timeline.
const DEFAULT_TIMELINE_VH = 32;
...
const DEFAULT_MEDIA_W = 300;
const MIN_MEDIA_W = 240;
const MAX_MEDIA_W = 520;

const DEFAULT_INSPECTOR_W = 300;
const MIN_INSPECTOR_W = 260;
const MAX_INSPECTOR_W = 480;
...
// Width of a side panel when collapsed to a thin strip (just the expand tab).
...
export const EditorInterface: React.FC = () => {
  ...
  // ── Panel collapse (gives the preview stage room to grow) ────────
  // Left media panel: manual collapse only.
  const [mediaCollapsed, setMediaCollapsed] = useState(false);
  // Right inspector panel: auto-collapses to a thin strip when nothing is
  // selected, but the user can override the current state with the toggle.
  const [inspectorCollapsed, setInspectorCollapsed] = useState(true);
  // Tracks whether the user has manually overridden the inspector's collapse
  // state for the CURRENT selection state, so auto-collapse doesn't fight them.
  const inspectorManualRef = useRef(false);
  const hasSelection = useUIStore((s) => s.selectedItems.length > 0);

  // Auto-collapse the inspector when the selection is cleared, and auto-expand
  // it when something is selected — unless the user just manually toggled it.
  useEffect(() => {
    if (inspectorManualRef.current) {
      // Consume the manual override once; the next selection change resumes auto.
  ...
  // AI Editor chat is disabled for now (will return as the agentic product layer).
  // Force it hidden regardless of stored panel state so no entry point can open it.
  ...
  // ── Responsive layout ───────────────────────────────────────────
  // Keep the side panels proportional to the viewport so the preview
  // stage always has usable room. On narrow screens the panels shrink
  // toward their minimums; on wide screens they grow back toward the
  // defaults. Runs on mount and on every window resize.
  useEffect(() => {
    const applyResponsiveWidths = () => {
      const w = window.innerWidth;
      const chatOpen = useUIStore.getState().panels.agentChat?.visible ?? false;

      // Target ~19% of width for each side panel, clamped to sane bounds.
      const mediaTarget = clamp(Math.round(w * 0.19), MIN_MEDIA_W, DEFAULT_MEDIA_W);
      const inspectorTarget = clamp(Math.round(w * 0.19), MIN_INSPECTOR_W, DEFAULT_INSPECTOR_W);
      const chatTarget = chatOpen
        ? clamp(Math.round(w * 0.2), MIN_CHAT_W, DEFAULT_CHAT_W)
        : 0;

      // Ensure the stage keeps at least MIN_STAGE_W. If not, shave the
      // panels down proportionally (media + inspector first).
      const handles = RESIZE_HANDLE * (chatOpen ? 3 : 2) + 60; // rail + padding budget
      let media = mediaTarget;
      let inspector = inspectorTarget;
      const stageRoom = w - media - inspector - chatTarget - handles;
      if (stageRoom < MIN_STAGE_W) {
        const deficit = MIN_STAGE_W - stageRoom;
        // Split the deficit between the two side panels, respecting mins.
        media = Math.max(MIN_MEDIA_W, media - Math.ceil(deficit / 2));
        inspector = Math.max(MIN_INSPECTOR_W, inspector - Math.ceil(deficit / 2));
      }

      setMediaWidth(media);
      setInspectorWidth(inspector);
      if (chatOpen && chatTarget > 0) setChatWidth(chatTarget);
    };
  ...
  const effectiveTimelineVh = timelineMaximized
    ? COMPACT_TIMELINE_VH
    : timelineVh;
  // Effective column widths: a collapsed panel shrinks to a thin strip and its
  // resize handle disappears (nothing to drag), freeing the space for the 1fr
  // stage column which grows to fill it.
  const mediaCol = mediaCollapsed ? COLLAPSED_PANEL_W : mediaWidth;
  const inspectorCol = inspectorCollapsed ? COLLAPSED_PANEL_W : inspectorWidth;
  const mediaHandle = mediaCollapsed ? 0 : RESIZE_HANDLE;
  const inspectorHandle = inspectorCollapsed ? 0 : RESIZE_HANDLE;
  const gridStyle: React.CSSProperties = chatVisible
    ? {
        gridTemplateColumns: `${mediaCol}px ${mediaHandle}px 1fr ${inspectorHandle}px ${inspectorCol}px ${RESIZE_HANDLE}px ${chatWidth}px`,
    ...
    : {
        gridTemplateColumns: `${mediaCol}px ${mediaHandle}px 1fr ${inspectorHandle}px ${inspectorCol}px`,
        gridTemplateRows: `1fr ${RESIZE_HANDLE}px ${effectiveTimelineVh}vh`,
        gridTemplateAreas:
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        >
        <div
          className="bg-bg-1 min-w-0 min-h-0 overflow-hidden rounded-xl border border-border shadow-sm relative"
        ...
        >
          {mediaCollapsed ? (
            <CollapsedStrip
              side="left"
              label="Media"
              onExpand={() => setMediaCollapsed(false)}
            />
          ) : (
            <>
              <PanelCollapseButton
