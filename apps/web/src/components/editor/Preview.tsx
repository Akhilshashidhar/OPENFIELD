...
export const Preview: React.FC = () => {
  ...
  const previewFrameSize = useMemo(() => {
    ...
    // Fill the available preview area edge-to-edge (only a hair of margin)
    // while preserving the project aspect ratio, so the video reads as ~100%
    // of the player rather than floating small in the center.
  ...
  return (
    ...
    >
      ...
      {/* Video Area — edge-to-edge stage (OpenField: video fills the whole player) */}
      <div
        ref={videoAreaRef}
        className={`flex-1 min-h-0 min-w-0 relative flex items-center justify-center bg-[var(--stage-bg)] transition-all duration-300 ${
      ...
      >
        <div
          ref={overlayRef}
          className={`relative bg-[var(--screen-bg)] overflow-visible transition-all duration-300 rounded-none ring-0 shadow-none`}
          style={
            isMaximized || isFullscreen
              ? {
                  width: "100%",
        ...
        >
          <canvas
            ...
            width={previewRes.width}
            height={previewRes.height}
            className="w-full h-full object-contain bg-[var(--screen-bg)] rounded-none"
            style={{
      ...
      >
        ...
        <div className="flex items-center gap-4 mx-auto">
          ...
          <IconButton
            ...
            size="md"
            onClick={() => {
              // If the playhead is parked at (or within a frame of) the end,
              // restart from the beginning when the user presses Play — so they
              // don't have to drag the playhead back to 00:00 manually.
              if (!isPlaying && actualEndTime > 0) {
                // editingFrameDuration is in ms; compare in seconds.
                const endEpsilon = Math.max(0.05, editingFrameDuration / 1000);
                if (playheadPosition >= actualEndTime - endEpsilon) {
                  seekTo(0);
                  startPositionRef.current = 0;
                  setPlayheadPosition(0);
                }
