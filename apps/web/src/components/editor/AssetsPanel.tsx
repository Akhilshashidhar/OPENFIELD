...
const ASSETS_TABS: ReadonlyArray<{
  ...
  // AI Generate, Recipes, and Project Templates are disabled for now (MVP1).
  // Their tab entries are removed from the rail; the switch cases below remain
  // as dead-but-valid code so they can be re-enabled by restoring these entries.
] as const;
...
export const AssetsPanel: React.FC = () => {
  ...
  const handleFileImport = useCallback(
    async (files: FileList | null) => {
      ...
      // Snapshot how many media items exist before import so we can find the
      // newly-added ones afterwards (for auto-add-to-timeline).
      ...
      try {
        for (let i = 0; i < fileArray.length; i++) {
          ...
          // Quiet, generic progress — no raw filename in the UI (point 2).
          setImportProgress(
            fileArray.length > 1
              ? `Importing ${i + 1} of ${fileArray.length}…`
              : "Importing…",
          );
          await importMedia(file);
        }

        // ── Auto-add to timeline (point 3) ──
        // Find items added during this import.
        const afterItems = useProjectStore
          .getState()
          .project.mediaLibrary.items.filter((m) => !beforeIds.has(m.id));

        // Single new import → place it on the timeline automatically.
        // Multiple → leave them in the library for the user to choose.
        if (afterItems.length === 1) {
          await addMediaToTimeline(afterItems[0]);
        }
      } catch (error) {
